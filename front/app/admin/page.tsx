import StatCards from '@/components/hr/admin/dashboard/StatCards';
import SystemLogs from '@/components/hr/admin/dashboard/SystemLogs';
import UsageChart from '@/components/hr/admin/dashboard/UsageChart';
...
const fetchDashboardData = async () => {
    ...
    return {
        metrics: {
            apiCalls: { value: '234,512', change: '+12.5%', isUp: true, label: '총 API 호출 수' },
            tokenUsage: { value: '45.2M', change: '+8.2%', isUp: true, label: '소모 토큰 (Tokens)' },
            estimatedCost: { value: '$1,240', change: '-2.4%', isUp: false, label: '이달 예상 청구액' },
            errorRate: { value: '0.12%', change: '-0.05%', isUp: false, label: '시스템 에러율' },
        },
        weeklyData: [
        ...
        logs: [
            { id: 'LOG-001', type: 'warning', message: '면접 질문 생성 중 토큰 한도 초과 위험', time: '10분 전', user: 'system' },
            { id: 'LOG-002', type: 'error', message: '이력서 PDF 파싱 실패 (지원자 ID: cnd_891)', time: '42분 전', user: '김인사' },
            { id: 'LOG-003', type: 'info', message: '새로운 RAG 지식 베이스 벡터화 완료', time: '2시간 전', user: '최관리' },
            { id: 'LOG-004', type: 'hallucination', message: '[환각 의심] 존재하지 않는 React 훅 언급 감지', time: '5시간 전', user: 'AI Agent' },
        ]
    };
};
        ...
        <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-700">
            ...
            <header className="border-b border-slate-200/60 pb-6">
                ...
                <p className="text-slate-500 font-semibold text-[14px] mt-1">
                    엔터프라이즈 AI 에이전트의 API 사용량, 인프라 비용 및 시스템 상태를 실시간으로 모니터링합니다.
                </p>
            </header>
