...
import InterviewerClient from "@/components/hr/interviewers/InterviewerClient";

export default async function InterviewersPage() {
  const [interviewerList, positions] = await Promise.all([
    fetchInterviewersServer(),
    fetchPositionsServer(),
  ]);

  const interviewerTotal =
    interviewerList.totalElements ?? interviewerList.content.length;
  ...
  return (
    <div className="flex h-full flex-col gap-4 animate-in fade-in duration-500 sm:gap-6">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between lg:gap-8">
          <div className="min-w-0 flex-1 space-y-3 sm:space-y-4">
            <p className="inline-flex max-w-full items-center gap-2 rounded-full border border-indigo-100/90 bg-white/90 px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-indigo-600 shadow-sm backdrop-blur-sm sm:text-[11px]">
              ...
              <span className="truncate">HR · 면접 운영</span>
            </p>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
              ...
              <div className="min-w-0 space-y-2">
                ...
                <p className="max-w-xl text-sm font-medium leading-relaxed text-slate-600 sm:text-[15px]">
                  ...
                  담당 직무·면접 차수·연락처를 한곳에서 정리하고, 일정 배정 시 바로
                  활용할 수 있게 유지하세요.
                </p>
              </div>
            </div>
      ...
      <div className="min-h-0 flex-1">
        <InterviewerClient
          initialData={interviewerList.content}
          positions={positions}
          listTotalCount={interviewerTotal}
        />
      </div>
    </div>
  );
