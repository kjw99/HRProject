        ...
        >
            ...
            >
                <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                        <h3 className="text-[20px] font-black text-slate-900">{event.title} 상세</h3>
                        <p className="text-[13px] text-slate-400 font-bold flex items-center gap-1 mt-1">
                            <i className='bx bx-time-five'></i> {event.startTime} - {event.endTime} | {event.location}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-8 space-y-6">
                    <h4 className="text-[14px] font-black text-slate-400 uppercase tracking-widest">참석 대상자 ({event.candidates.length}명)</h4>
                    <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                        {event.candidates.map(candidate => (
                            <div key={candidate.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center font-black">
                                        {candidate.name[0]}
                                    </div>
                                    <div>
                                        <p className="text-[14px] font-bold text-slate-800">{candidate.name}</p>
                                        <p className="text-[12px] text-slate-400 font-medium">{candidate.position}</p>
                                    </div>
                                </div>
                                <button className="text-indigo-500 hover:text-indigo-700 text-sm font-black">이력서 보기</button>
                            </div>
                        ))}
                    </div>
