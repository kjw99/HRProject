...
const CandidateCard: React.FC<CandidateCardProps> = ({ candidate, viewMode = 'detailed', onEdit, onDelete, isSelected, onToggleSelect }) => {
  ...
  return (
    <div
      ...
      className={`rounded-[24px] border shadow-[0_4px_15px_rgba(0,0,0,0.03)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 cursor-grab active:cursor-grabbing group flex flex-col relative z-10 ${viewMode === 'compact' ? 'p-4' : 'p-5'} ${isSelected ? 'bg-indigo-50/60 border-indigo-400 ring-2 ring-indigo-500/20' : 'bg-white border-slate-200/80 hover:border-indigo-300'}`}
    >
      <div className={`flex justify-between items-start pointer-events-none ${viewMode === 'compact' ? 'mb-0' : 'mb-4'}`}>
        <div className="flex items-center gap-3">
          {/* 🚀 아바타 영역 (선택 시 체크마크로 변환) */}
          <div className={`rounded-[14px] flex items-center justify-center font-black border shadow-sm transition-all duration-300 ${viewMode === 'compact' ? 'w-9 h-9 text-base' : 'w-11 h-11 text-lg'} ${isSelected ? 'bg-indigo-500 text-white border-indigo-600 scale-105' : 'bg-slate-100 text-slate-500 border-slate-200/60'}`}>
            {isSelected ? <i className='bx bx-check text-2xl animate-in zoom-in duration-200'></i> : candidate.name.charAt(0)}
          </div>
          <div>
            <h4 className={`font-black leading-tight transition-colors ${viewMode === 'compact' ? 'text-[14px]' : 'text-[16px]'} ${isSelected ? 'text-indigo-900' : 'text-slate-900'}`}>{candidate.name}</h4>
            <p className={`text-[11px] font-bold mt-1 uppercase tracking-wider transition-colors ${isSelected ? 'text-indigo-500' : 'text-slate-400'}`}>ID: {candidate.id}</p>
          </div>
        </div>
        ...
        <div className="relative pointer-events-auto shrink-0">
          <button
            ...
            className={`opacity-0 group-hover:opacity-100 transition-opacity w-8 h-8 rounded-full flex items-center justify-center ${isSelected ? 'text-indigo-600 bg-indigo-100 hover:bg-indigo-200' : 'text-slate-300 bg-slate-50 hover:bg-indigo-50 hover:text-indigo-500'}`}
          >
