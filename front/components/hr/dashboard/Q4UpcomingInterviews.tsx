            ...
            <div className="p-6 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 flex-1">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {items.map((item) => {
                        ...
                        // 💡 1. 현재 시간과 면접 시간의 차이를 계산 (밀리초 단위)
                        const now = new Date();
                        const diffTime = d.getTime() - now.getTime();

                        // 💡 2. 밀리초를 '일(Day)' 단위로 변환 (1일 = 1000ms * 60초 * 60분 * 24시간)
                        const diffDays = diffTime / (1000 * 60 * 60 * 24);

                        // 💡 3. 일주일(7일) 이내인지 판별 (과거 날짜도 포함하고 싶다면 diffDays <= 7 만 써도 됩니다)
                        const isUrgent = diffDays > 0 && diffDays <= 7;
                        return (
                            <div key={item.id} className="border border-slate-200 rounded-2xl p-4 hover:border-indigo-200 hover:shadow-sm transition-all flex flex-col bg-white">
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <span className="text-xs font-black bg-slate-100 text-slate-600 px-2 py-0.5 rounded uppercase tracking-wide mb-1.5 inline-block">
                                            {item.team}
                                        </span>
                                        <h3 className="font-bold text-slate-800 text-sm">{item.round}</h3>
                                        <p className={`text-xs font-bold mt-1 ${isUrgent ? 'text-rose-500' : 'text-slate-700'}`}>
                                            <i className="bx bx-calendar-event"></i> {formattedDate}
                                            {/*isUrgent && <span className="ml-1.5 px-1.5 py-0.5 bg-rose-100 text-rose-600 rounded text-[10px] uppercase">임박</span>*/}
                                        </p>
                                    </div>
                                    <button className="text-xs font-bold bg-indigo-50 text-indigo-600 px-2.5 py-1.5 rounded-lg hover:bg-indigo-100 transition-colors flex items-center gap-1">
                                        <i className="bx bx-user-plus"></i> 면접관 할당
                                    </button>
                                </div>
