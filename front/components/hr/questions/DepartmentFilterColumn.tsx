              ...
              return (
                <li key={dept.positionId}>
                  <button
                    ...
                    className={`flex w-full items-center justify-between gap-3 rounded-xl px-3 py-3 text-left transition sm:px-3.5 sm:py-3.5 ${selected
                        ? "bg-linear-to-r from-indigo-600 to-violet-600 text-white shadow-md shadow-indigo-300/30"
                        : "text-slate-700 hover:bg-slate-50 active:scale-[0.99]"
                      }`}
                  >
                    <span className="flex min-w-0 flex-1 items-start gap-3">
                      <span
                        className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-lg ${selected
                      ...
                      <span className="min-w-0 flex-1">
                        <span
                          className={`block truncate text-sm font-bold ${selected ? "text-white" : "text-slate-900"
                            }`}
                        >
                          {dept.positionName}
                        </span>
                        <span
                          className={`mt-0.5 block text-[11px] font-medium ${selected ? "text-indigo-100" : "text-slate-400"
                            }`}
                        >
                          ID {dept.positionId}
                        </span>
                    ...
                    <span className="flex shrink-0 items-center gap-2">
                      <span
                        className={`tabular-nums text-sm font-black ${selected ? "text-white" : "text-indigo-600"
                          }`}
                        title="저장된 질문 수"
                      >
                        {isQuestionCountLoading ? (
                          <span
                            className={`inline-block h-4 w-8 shrink-0 animate-pulse rounded ${selected ? "bg-white/35" : "bg-slate-200/90"
                              }`}
                            aria-hidden
                          />
                        ) : (
                          <>
                            ...
                            <span
                              className={`ml-0.5 text-[11px] font-bold ${selected ? "text-indigo-100" : "text-slate-400"
                                }`}
                            >
                              개
                            </span>
