...
def build_question_selection_messages(
    ...
    human_prompt = f"""
아래 후보 질문 목록에서 최종 질문을 정확히 {data.question_count}개 선택하세요.

지원 직무: {data.position_name}
이번 호출에서 선택할 질문 개수: {data.question_count}
...
- 실제 프로젝트, 성과, 문제 해결, 의사결정, 리스크 대응을 확인할 수 있는 질문을 우선하세요.
- 이미 재사용된 질문과 중복되거나 매우 유사한 질문은 선택하지 마세요.
- 보호 대상 개인정보나 직무와 무관한 사적 정보는 제외하세요.
- 최종 결과는 정확히 {data.question_count}개여야 합니다.
- 선택 결과는 질문 본문과 평가 의도, 생성 근거를 유지하세요.
- question_keywords는 이 단계에서 판단하지 않습니다. 후속 단계에서 별도로 매핑합니다.
""".strip()
