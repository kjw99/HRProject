...
def build_fit_analysis_messages(
    ...
    human_prompt = f"""
아래 지원자 정보와 직무기술서 정보를 비교하여 면접 질문 생성에 필요한 핵심 분석 요소를 도출하세요.
...
## 이미 재사용된 질문 수
{optional_text(str(len(data.reused_questions)) if data.reused_questions else None)}

분석 기준:
- 직무기술서에서 반드시 검증해야 할 핵심 역량과 요구사항을 식별하세요.
- 이력서에서 해당 요구사항과 직접 연결되는 경험, 프로젝트, 기술, 성과 근거를 찾으세요.
- 근거가 부족하거나 모호한 부분은 면접에서 확인해야 할 리스크 또는 갭으로 정리하세요.
- 질문 생성 우선순위가 높은 주제를 구체적으로 제안하세요.
